...
#  tmp2 <- ifelse(km2 > 0, 1 / km2, 0)
  ...
  ip.weight <- (censoring_status==0) * tmp1
...
calculateAJ <- function(t, epsilon, ip.weight, code.event=1) {
  d <- as.integer(epsilon==code.event)
  atrisk <- outer(t, t, "<=")
  km <- atrisk %*% as.matrix(d*ip.weight)/length(t)
  return(km)
}
ip.weight <- calculateIPCW(Surv(t,d)~1, diabetes.complications, code.censoring=0, strata_name=NULL, specific.time=NULL)
km <- calculateAJ(diabetes.complications$t, diabetes.complications$epsilon, ip.weight)

strata <- rep(1, nrow(diabetes.complications))
out_km <- calculateKM_rcpp(t=diabetes.complications$t, d=diabetes.complications$d, strata=strata, error="none")
print(out_km$surv)
print(out_km$time)
