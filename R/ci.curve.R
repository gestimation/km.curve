...
ci.curve <- function(formula, data, weights = NULL, subset = NULL,
  ...
  if (any(as.integer(out_readSurv$strata) != 1)) {
    n_tab <- as.integer(table(as.integer(out_readSurv$strata)))     # 各ストラタのサンプル数
    rep_list <- mapply(rep, n_tab, out_aj$strata1, SIMPLIFY = FALSE)
    n.risk <- do.call(c, rep_list) - out_aj$n.cum.censor - out_aj$n.cum.event1 - out_aj$n.cum.event2
  } else {
    n_tab <- length(out_readSurv$strata)
    n.risk <- n_tab - out_aj$n.cum.censor - out_aj$n.cum.event1 - out_aj$n.cum.event2
  }
  ...
  if (any(as.integer(out_readSurv$strata) != 1)) {
    ...
    for (s in seq_along(cif_len)) {
      idx_cif <- (cif_breaks[s] + 1):cif_breaks[s + 1]
      idx_km  <- (km_breaks[s]  + 1):km_breaks[s + 1]
  ...
  if (report.survfit.std.err) {
    out_aj$std.err <- out_aj$std.err / pmax(out_aj$surv, .Machine$double.eps)
  }

  survfit_object <- list(
    time   = out_aj$time1,
    surv   = out_aj$surv,
    n      = if (any(as.integer(out_readSurv$strata) != 1)) table(as.integer(out_readSurv$strata)) else length(out_readSurv$strata),
    n.risk = n.risk,
...
calculateAJ <- function(data) {
  ...
  return(list(time1=time1, aj1=aj1, n.event1=n.event1, n.event2=n.event2, n.censor=n.censor, n.cum.event1=n.cum.event1, n.cum.event2=n.cum.event2, n.cum.censor=n.cum.censor, strata1=strata1, time0=out_km0$time, km0=out_km0$surv, km0_strata=out_km0$strata))
}
